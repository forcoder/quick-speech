...
interface WubiDao {
    ...
    suspend fun prefixMatch(pattern: String, limit: Int = 50): List<WubiWordEntry>
    ...
    /** 前缀匹配，返回Flow */
    @Query("SELECT * FROM wubi_words WHERE code LIKE :pattern ORDER BY frequency DESC LIMIT :limit")
    fun prefixMatchFlow(pattern: String, limit: Int = 50): Flow<List<WubiWordEntry>>
    ...
    @Query("SELECT * FROM wubi_words WHERE word LIKE :pattern AND type > 0 ORDER BY frequency DESC LIMIT :limit")
    suspend fun associateWords(pattern: String, limit: Int = 20): List<WubiWordEntry>
